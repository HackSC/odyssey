...
module.exports = {
    ...
    try {
      models.HackerProfile.findAll({
        ...
        if (hackerProfiles.length >= 1) {
          if (
            hackerProfiles[0].get("role") === "admin" ||
            hackerProfiles[0].get("role") === "superadmin"
          ) {
            return next();
          } else {
  ...
  requireJudge: function (req, res, next) {
    /* Read user from database, and make sure it is in fact an Judge */
    try {
      models.HackerProfile.findAll({
        where: {
          userId: req.user.id,
        ...
        if (hackerProfiles.length >= 1) {
          if (hackerProfiles[0].get("role") === "judge") {
            return next();
          } else {
            res.status(400).send("Unauthorized: Incorrect role");
          }
